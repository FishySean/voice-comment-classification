...
使用方法:
  python3 run_binary_classify.py
  python3 run_binary_classify.py --batch-size 10
"""
...
# 默认路径
DEFAULT_PROMPT = "/Users/para_fish66/Desktop/语音评论/prompt/social_interaction_detection/版本2.txt"
DEFAULT_INPUT = "/Users/para_fish66/Desktop/语音评论/data/testing_data/social_interaction_detection/评测集0606_v2.csv"
DEFAULT_EVAL_DIR = "/Users/para_fish66/Desktop/语音评论/data/output_data/social_interaction_detection/eval_data"
DEFAULT_DIFF_DIR = "/Users/para_fish66/Desktop/语音评论/data/output_data/social_interaction_detection/diff_data"
# ==============================
...
def build_batch_prompt(prompt_template: str, batch: list[dict[str, str]]) -> str:
    ...
    samples_block += (
        ...
        "```json\n"
        "[\n"
        '  {"voiceId": "<原样回填>", "voice_asr_text": "<原样回填>", "label": 0或1, "label_name": "社交互动"或"非社交互动", "reason": "<不超过80字的中文理由>"},\n'
        "  ...\n"
        "]\n"
        "```\n"
...
def compute_accuracy_and_diff(input_path: str, eval_path: str, diff_dir: str):
    ...
    for pred in preds:
        ...
        if pred_label != true_label:
            true_name = "社交互动" if true_label == "1" else "非社交互动"
            diff_rows.append({
                "voice_resource_id": vid,
                "voice_asr_text": pred.get("voice_asr_text", ""),
                "true_label": true_label,
...
def main():
    parser = argparse.ArgumentParser(description="语音评论「社交互动」二分类批量评测脚本")
    parser.add_argument(
    ...
    parser.add_argument(
        "--run-name", type=str, default="第三次评测（prompt版本2，评测集0606_v2）",
        help="本次评测名称（默认: 第三次评测（prompt版本2，评测集0606_v2））"
    )
    parser.add_argument(
    ...
    for start in range(0, total, batch_size):
        ...
        neg = labels.count("0")
        err = labels.count("-1")
        print(f"  ✅ 完成: 社交互动={pos}, 非社交互动={neg}, 异常={err}")
