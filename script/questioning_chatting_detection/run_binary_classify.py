...
DEFAULT_PROMPT = "/Users/para_fish66/Desktop/语音评论/prompt/questioning_chatting_detection/版本2.txt"
DEFAULT_INPUT = "/Users/para_fish66/Desktop/语音评论/data/testing_data/questioning_chatting_detection/评测集0606_v2.csv"
DEFAULT_EVAL_DIR = "/Users/para_fish66/Desktop/语音评论/data/output_data/questioning_chatting_detection/eval_data"
DEFAULT_DIFF_DIR = "/Users/para_fish66/Desktop/语音评论/data/output_data/questioning_chatting_detection/diff_data"
# ==============================
...
def build_batch_prompt(prompt_template: str, batch: list[dict[str, str]]) -> str:
    ...
    samples_block += (
        ...
        "```json\n"
        "[\n"
        '  {"voiceId": "<原样回填>", "voice_asr_text": "<原样回填>", "label": 0或1, "label_name": "提问/互动"或"非提问/互动", "reason": "<不超过80字的中文理由>"},\n'
        "  ...\n"
        "]\n"
        "```\n"
...
def compute_accuracy_and_diff(input_path: str, eval_path: str, diff_dir: str):
    ...
    for row in original:
        ...
        if vid:
            truth_map[vid] = {"true_label": truth_label, "intent": intent, "asr_text": asr}

    preds = load_csv_data(eval_path)
    total = 0
    correct = 0
    diff_rows: list[dict[str, str]] = []
    tp1 = fp1 = fn1 = tn1 = real_1_total = pred_1_total = 0
    ...
    for pred in preds:
        ...
        if pred_label == "-1":
            total += 1
            diff_rows.append({
                "voice_resource_id": vid, "voice_asr_text": pred.get("voice_asr_text", ""),
                "true_label": true_label, "pred_label": "异常",
                "true_label_name": truth["intent"], "pred_label_name": pred_label_name,
                "pred_reason": pred_reason, "error_type": "API异常",
            })
            continue
        ...
        if true_label == "1":
            real_1_total += 1
            if pred_label == "1":
                tp1 += 1; pred_1_total += 1
            else:
                fn1 += 1
        else:
            if pred_label == "1":
                fp1 += 1; pred_1_total += 1
            else:
                tn1 += 1

        if pred_label != true_label:
            true_name = "提问/互动" if true_label == "1" else "非提问/互动"
            diff_rows.append({
                "voice_resource_id": vid, "voice_asr_text": pred.get("voice_asr_text", ""),
                "true_label": true_label, "pred_label": pred_label,
                "true_label_name": true_name, "pred_label_name": pred_label_name,
                "pred_reason": pred_reason, "error_type": "误判",
            })
    ...
    diff_fieldnames = [
        "voice_resource_id", "voice_asr_text", "true_label", "pred_label",
        "true_label_name", "pred_label_name", "pred_reason", "error_type",
    ]
    ...
    summary_text = (
        ...
        f"FP(真实0→预测1): {fp1}\n"
        f"FN(真实1→预测0): {fn1}\n"
        f"精确率(Precision): {tp1}/{tp1+fp1} = {precision_1:.2f}%\n"
        f"召回率(Recall):     {tp1}/{tp1+fn1} = {recall_1:.2f}%\n"
        f"F1:                 {f1_1:.2f}%\n"
    )
...
def main():
    parser = argparse.ArgumentParser(description="语音评论「提问/互动」二分类批量评测脚本")
    parser.add_argument("--prompt", type=str, default=DEFAULT_PROMPT, help="Prompt 模板文件路径")
    parser.add_argument("--input", type=str, default=DEFAULT_INPUT, help="评测集 CSV 文件路径")
    parser.add_argument("--eval-dir", type=str, default=DEFAULT_EVAL_DIR, help="评测结果输出目录")
    parser.add_argument("--diff-dir", type=str, default=DEFAULT_DIFF_DIR, help="diff 文件输出目录")
    parser.add_argument("--run-name", type=str, default="第三次评测（prompt版本2，评测集0606_v2）", help="本次评测名称")
    parser.add_argument("--batch-size", type=int, default=BATCH_SIZE, help=f"每次请求处理的数据条数（默认 {BATCH_SIZE}）")
    ...
    if os.path.exists(diff_subdir):
        try:
            ...
            all_files = []
        summary_files = [f for f in all_files if f.startswith(input_stem) and "_summary_" in f and f.endswith(".txt")]
        if summary_files:
            print(f"⏭️  检测到 diff_data/{args.run_name}/ 下已有 summary 文件，本轮评测已完成，跳过")
            return
    ...
    for start in range(0, total, batch_size):
        ...
        print(f"\n🔄 批次 {batch_num}/{total_batches} | 原始范围 [{start + 1}-{end}] | 实际处理 {len(remaining_batch)} 条")
        ...
        neg = labels.count("0")
        err = labels.count("-1")
        print(f"  ✅ 完成: 提问/互动={pos}, 非提问/互动={neg}, 异常={err}")
    ...
    if len(done_ids) < total:
        print(f"\n⚠️  还有 {total - len(done_ids)} 条数据未处理")
